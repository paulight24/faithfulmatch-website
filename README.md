...
1. Find-and-replace `https://faithfulmatch.love/` → `https://faithfulmatch.love/` across every `index.html` file's `<link rel="canonical">` and `og:url`/`og:image` tags (only 7 files, one line each — grep for `empoweredforwealth.com` to find them all).
